...
class SamplingDialog extends StatefulComponent {
  ...
  const SamplingDialog({required this.prompt, required this.onSubmit, required this.onCancel, super.key});
    ...
    return div(classes: 'dialog-overlay', [
      div(classes: 'dialog dialog-wide', [
        div(classes: 'dialog-header', [
          ...
          button(classes: 'btn-close', onClick: component.onCancel, [Component.text('×')]),
        ]),
        div(classes: 'dialog-content', [
          div(classes: 'dialog-icon sampling-icon', [Component.text('✨')]),
          p(classes: 'dialog-label', [Component.text('Server is asking for:')]),
          ...
          if (!_useMockResponse)
            div(classes: 'form-group', [
              label(attributes: {'for': 'response-input'}, [Component.text('Your response:')]),
              textarea(
                id: 'response-input',
                attributes: {'rows': '4', 'placeholder': 'Enter your response here...'},
                events: {
                  'input': (event) {
        ...
        div(classes: 'dialog-actions', [
          button(classes: 'btn btn-secondary', onClick: component.onCancel, [Component.text('Cancel')]),
          button(classes: 'btn btn-primary', onClick: _handleSubmit, [Component.text('Submit Response')]),
        ]),
      ]),
    ]);
  }
