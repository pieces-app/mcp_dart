...
class McpClientScreenState extends State<McpClientScreen> {
  ...
  void initState() {
    ...
    _responseText = 'Welcome to the MCP Client.\nClick Connect to connect to the server.';
  ...
  Widget _buildCommandButton(
    ...
    return ElevatedButton(
      onPressed: (!requiresConnection || widget.mcpService.isConnected) ? onPressed : null,
      style: style ?? ElevatedButton.styleFrom(minimumSize: const Size(120, 40)),
      child: Text(label),
    );
  }
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      _setResponse(result ? 'Connected to MCP server' : 'Failed to connect: ${widget.mcpService.connectionError}');
    } catch (e) {
  ...
  Future<void> _reconnect() async {
    ...
    try {
      ...
      _setResponse(result ? 'Reconnected to server' : 'Failed to reconnect: ${widget.mcpService.connectionError}');
    } catch (e) {
  ...
  Future<void> _listTools() async {
    ...
    try {
      ...
      if (tools == null || tools.isEmpty) {
        ...
        final toolsText = tools.map((t) => '- ${t.name}: ${t.description}').join('\n');
        ...
        if (_selectedTool == null && tools.isNotEmpty) {
          ...
          final commonTool = tools.firstWhere((t) => commonTools.contains(t.name), orElse: () => tools.first);
  ...
  Future<void> _listPrompts() async {
    ...
    try {
      ...
      if (prompts == null || prompts.isEmpty) {
        ...
        final promptsText = prompts.map((p) => '- ${p.name}: ${p.description}').join('\n');
  ...
  Future<void> _listResources() async {
    ...
    try {
      ...
      if (resources == null || resources.isEmpty) {
        ...
        final resourcesText = resources.map((r) => '- ${r.name}: ${r.uri}').join('\n');
  ...
  void _clearNotifications() {
    setState(() {
      ...
      widget.mcpService.refresh(); // Use refresh method instead of directly calling notifyListeners
    });
  }
  ...
  void _showServerConfigDialog() {
    final serverController = TextEditingController(text: widget.mcpService.serverUrl);
  ...
  Future<void> _showErrorDialog(String title, String message, {Object? error}) async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(title),
        content: SingleChildScrollView(child: Text('$message$errorDetails')),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.settings), onPressed: _showServerConfigDialog, tooltip: 'Configure Server'),
          Padding(
            ...
            child: Chip(
              ...
              backgroundColor: widget.mcpService.isConnected ? Colors.green : Colors.red,
            ),
          ),
        ],
      ),
          ...
          children: [
            ...
            Padding(
              ...
              child: Wrap(
                ...
                runSpacing: 8.0,
                children: [
                  _buildCommandButton('Connect', _connect, requiresConnection: false),
                  _buildCommandButton('Disconnect', _disconnect),
                  _buildCommandButton('Terminate Session', _terminateSession),
                  _buildCommandButton('Reconnect', _reconnect, requiresConnection: false),
                  ...
                  _buildCommandButton('Clear Notifications', _clearNotifications, requiresConnection: false),
                ],
              ),
            ),
                ...
                children: [
                  Expanded(
                    ...
                    child: Builder(
                      builder: (context) {
                        ...
                        if (_selectedTool != null && !availableTools.any((item) => item.value == _selectedTool)) {
                          _selectedTool = null;
                        }

                        return DropdownButtonFormField<String>(
                          decoration: const InputDecoration(labelText: 'Tool', border: OutlineInputBorder()),
                          initialValue: _selectedTool,
                          items: availableTools,
                          onChanged: widget.mcpService.isConnected
                              ? (value) {
                                  if (value != null) {
                                    setState(() {
                                      _selectedTool = value;
                  ...
                  Expanded(
                    child: OutlinedButton(
                      onPressed: widget.mcpService.isConnected ? _callTool : null,
                      style: OutlinedButton.styleFrom(minimumSize: const Size(120, 58)),
            ...
            if (widget.mcpService.availablePrompts != null && widget.mcpService.availablePrompts!.isNotEmpty)
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16.0),
                child: Row(
                  children: [
                    Expanded(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        initialValue: _selectedPrompt.isEmpty && widget.mcpService.availablePrompts!.isNotEmpty
                            ? widget.mcpService.availablePrompts!.first.name
                            : _selectedPrompt,
                        items: widget.mcpService.availablePrompts!
                    ...
                    Expanded(
                      child: OutlinedButton(
                        onPressed: widget.mcpService.isConnected ? _getPrompt : null,
                        style: OutlinedButton.styleFrom(minimumSize: const Size(120, 58)),
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  labelText: _selectedTool == 'greet' || _selectedTool == 'multi-greet'
                      ...
                      child: Stack(
                        children: [
                          if (_isLoading)
                            ...
                            SingleChildScrollView(controller: _scrollController, child: SelectableText(_responseText)),
                        ],
                      ),
                    ),
                  ],
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final notification =
                              widget.mcpService.notifications[widget.mcpService.notifications.length - 1 - index];
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              color: Colors.white,
                              border: Border.all(
                                color: notification.level.contains('error')
                                    ? Colors.red
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: notification.level.contains('error')
                                            ...
                                            : notification.level.contains('warn')
                                            ? Colors.orange
                                            : notification.level.contains('info')
                                            ? Colors.blue
                                            : Colors.grey,
                                        borderRadius: BorderRadius.circular(4),
                                      ),
