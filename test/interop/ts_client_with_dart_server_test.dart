...
void main() {
  ...
  final tsClientPath = p.join(Directory.current.path, 'test/interop/ts/dist/client.js');
  final dartServerPath = p.join(Directory.current.path, 'test/interop/test_dart_server.dart');

  // Check if we should skip
  final skipTests = !File(tsClientPath).existsSync() || !File(dartServerPath).existsSync();
  ...
  group('TS Client with Dart Server', () {
    ...
    test('Stdio Transport', () async {
      ...
      expect(result.exitCode, equals(0), reason: 'TS Client failed in Stdio mode');
    });
    ...
    test('Streamable HTTP Transport', () async {
      ...
      print('Server started on port $port with (expected) session $sessionId');

      try {
        // Start TS Client
        try {
          print("Starting TS Client with Session ID $sessionId...");
          final clientProcess = await Process.start('node', [
            tsClientPath,
            '--transport',
            'http',
            '--url',
          ...
          clientProcess.stdout.transform(utf8.decoder).listen((data) => print('[Client Output] $data'));
          clientProcess.stderr.transform(utf8.decoder).listen((data) => print('[Client Error] $data'));
          ...
          expect(exitCode, equals(0), reason: 'TS Client failed in HTTP mode');
        } catch (e) {
