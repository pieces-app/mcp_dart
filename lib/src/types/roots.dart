...
class Root {
  ...
  Root({required this.uri, this.name}) : assert(uri.startsWith("file://"));

  factory Root.fromJson(Map<String, dynamic> json) {
    return Root(uri: json['uri'] as String, name: json['name'] as String?);
  }
...
class JsonRpcListRootsRequest extends JsonRpcRequest {
  const JsonRpcListRootsRequest({required super.id}) : super(method: Method.rootsList);

  factory JsonRpcListRootsRequest.fromJson(Map<String, dynamic> json) => JsonRpcListRootsRequest(id: json['id']);
}
...
class ListRootsResult implements BaseResultData {
  ...
  factory ListRootsResult.fromJson(Map<String, dynamic> json) {
    ...
    return ListRootsResult(
      roots: (json['roots'] as List<dynamic>?)?.map((r) => Root.fromJson(r as Map<String, dynamic>)).toList() ?? [],
      meta: meta,
    );
  }

  @override
  Map<String, dynamic> toJson() => {'roots': roots.map((r) => r.toJson()).toList()};
}
...
class JsonRpcRootsListChangedNotification extends JsonRpcNotification {
  const JsonRpcRootsListChangedNotification() : super(method: Method.notificationsRootsListChanged);

  factory JsonRpcRootsListChangedNotification.fromJson(Map<String, dynamic> json) =>
